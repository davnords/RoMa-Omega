...
# from .patch_embed import PatchEmbed as _
...
class MatchTransformer(nn.Module):
    def __init__(
        ...
        **ignored_kwargs,
        ...
        super().__init__()
        assert n_storage_tokens == 0, (
            "storage tokens are not supported for dinov3 init weights"
        )
        if in_dim != dim:
    ...
    def get_intermediate_layers(self, x: Tensor, n: int | list) -> list[Tensor]:
        ...
        for idx, blk in enumerate(self.blocks):
            ...
            if idx in n:
                outputs.append(
                    self.output_projector(self.norm(x)).reshape(
                        B, 2, H, W, self.out_dim
                    )
                )
        return outputs
...
ViTModel = Literal[
    ...
    "vit_giant2",
    "vit_7b",
    "vit_large_half_dinov3",
    "vit_base_mum_decoder",
]
