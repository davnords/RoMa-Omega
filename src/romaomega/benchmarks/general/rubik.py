...
from romaomega.geometry import pose_auc
...
class RubikBenchmark:
    ...
    def benchmark(self, model: BenchmarkMatcher):
        ...
        # Per-difficulty-level errors keyed by box label
        box_pose_err: dict[str, list] = {}
        box_t_err_metric: dict[str, list] = {}
        box_success: dict[str, int] = {}
        ...
        for scene in tqdm(all_scenes):
            ...
            for box in (pbar := tqdm(data, leave=False)):
                ...
                # Parse box key "[[ov_lo, ov_hi], [sc_lo, sc_hi], [vp_lo, vp_hi]]"
                (ov_lo, ov_hi), (sc_lo, sc_hi), (vp_lo, vp_hi) = eval(box)
                box_label = f"ov{ov_lo}-{ov_hi}_sc{sc_lo}-{sc_hi}_vp{vp_lo}-{vp_hi}"
                if box_label not in box_pose_err:
                    box_pose_err[box_label] = []
                    box_t_err_metric[box_label] = []
                    box_success[box_label] = 0
                    box_total[box_label] = 0
                ...
                for i, pair in enumerate(paths):
                    ...
                    mkpts1 = mkpts1 - model.offset
                    mkpts2 = mkpts2 - model.offset
                    ...
                    box_pose_err[box_label].append(max(R_err, t_err_angle))
                    box_t_err_metric[box_label].append(t_err_metric)
                    box_total[box_label] += 1

                    total += 1
                    if (R_err < 5.0) and (t_err_metric < 2.0):
                        success += 1
                        box_success[box_label] += 1
                    pbar.set_postfix(
        ...
        # Per-difficulty-level metrics (keyed by overlap/scale/viewpoint bin label)
        pose_thresholds = [5, 10, 20]
        for lbl, errs in box_pose_err.items():
            if not errs:
                continue
            box_auc = pose_auc(errs, pose_thresholds)
            n = box_total[lbl]
            sr = box_success[lbl] / n if n > 0 else 0.0
            results[f"{self.prefix}_{lbl}_n"] = n
            results[f"{self.prefix}_{lbl}_auc_5"] = box_auc[0] * 100
            results[f"{self.prefix}_{lbl}_auc_10"] = box_auc[1] * 100
            results[f"{self.prefix}_{lbl}_auc_20"] = box_auc[2] * 100
            results[f"{self.prefix}_{lbl}_success_ratio"] = sr * 100
